 ...
 Calendar();
			...
			<div style="padding:6px;margin-top:10px;">
					<ul>
							...
							<li><a href="http://blog.fens.me/nodejs-express3/">Node开发框架</a>
							<li><a href="http://nodeapi.ucdok.com/#/api/">Node.js  API教程</a>
							<li><a href="http://www.open-open.com/lib/view/1392611872538">7天学会Node.js</a>							
					</ul>		
			</div>
			</div>	
...
<h3>Node.js教程</h3>
<span>
在一个test.js文件里面写下：console.log("hell world");
</br>node test.js运行该js文件</br>
打开终端，键入node进入命令交互模式，可以输入一条代码语句后立即执行并显示结果</br>
<pre class="brush:javascript;gutter:true">
var http=require("http");
http.createServer(function(request,response){
response.writeHead(200,{"Content-Type":"text/plain"});
response.end("hello word! yudesong!");
}).listen(8080);
...
(1)安装node.js</br>
(2)安装Express </br>
	c:\node>npm install -g express </br>
(3)创建一个Express项目 </br>
	express  nodetest1 </br>
(4)编辑依赖项package.json </br>
	"ejs":"*", </br>
	"mongodb":"*", </br>
	"monk":"*" </br>
(5)安装依赖包</br>
...
(7) Ejs模板使用</br>
	...
	添加：app.engine('.html', ejs.__express);</br>
	修改：app.set('view engine', 'html');</br>
	重命名：views/indes.ejs 为 views/index.html</br>
	var mongo = require('mongodb');</br>
	var monk = require('monk');</br>
	var db = monk('localhost:27017/users');</br>
(8) 创建数据库并读取数据</br>
	[1] 安装Mongodb</br>
	[2] 运行Mongod和mongo</br>
	mongod –dbpath c:\node\nodetest1\data </br>
	...
	[3] 创建一个数据库并添加数据</br>
	use users</br>
	db.userscollection.insert({"username":"yudesong","password":"passwd"});</br>
	[4]查找数据</br>
	db.userscollection.find().pretty();</br>
	db.userscollection.find({"username":"yudesong"}).pretty();</br>
(9)添加页面:</br>
	添加index.html</br>	
	在index.js添加：
	<pre class="brush:javascript;gutter:true">
	router.regest = function(req, res){
	res.render('regest',{title:'用户注册'});
	};
	</pre>
	添加路由</br>
(10)添加登陆方法</br>
...
添加路由</br>
app.post('/dologin',routes.Login(db));</br>
</span>
</div>
