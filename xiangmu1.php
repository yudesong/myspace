...
<div class="entry">
<h1>基于SSH的中山大学考勤系统</h1>
<p>该项目前端采用Bootstrap框架,验证码插件,Ajax技术实现异步刷新。</p>
<p>后台采用EasyUI框架实现对用户列表的CURD操作。数据对象采用JSON格式传输.数据库采用Hibernate自动建表技术实现。</p>
<h3>搭建SSH开发环境</h3>
<p>Spring 配置sessionFactory</p>
...
public boolean UserLogin(User user){
	String msg = null;
	String hql = "from User where name=? and passwd=?";
	Query query = session.createQuery(hql);
	query.setParameter(0, user.getName());
	query.setParameter(1, user.getPasswd());
	List&lt;User&gt; lists = query.list();
	for(User message:lists)
	{
		msg = String.valueOf(message.getId());
	}
...
public class LoginAction extends ActionSupport {
	public String Login(){
		...
		if(mydb.UserLogin(user)) return SUCCESS;
		else{
...
<h3>项目展示</h3>
<img src="images/xiangmu1-1.png">
<img src="images/xiangmu1-2.png">
<img src="images/xiangmu1-3.png">
<img src="images/xiangmu1-4.png">
</div>
</div>
	</div>
