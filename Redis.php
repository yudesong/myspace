...
<strong>Redis与其他key-value存储有什么不同？</strong>
	<ul>
...
<strong>Redis 安装</strong></br>
<span>
(1)打开一个cmd窗口 使用cd命令切换目录到 C:\redis 运行 redis-server.exe redis.conf 。</br>
(2)切换到redis目录下运行 redis-cli.exe -h 127.0.0.1 -p 6379 。</br>
(3)设置键值对 set myKey abc</br>
(4)取出键值对 get myKey</br>
</span></br>
<strong>Redis 数据类型</strong>
<p>Redis支持五种数据类型：string（字符串），hash（哈希），list（列表），set（集合）及zset(sorted set：有序集合)。
