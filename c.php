...
<div class="entry">
<p><strong>C Hello World 实例</strong></p>
C 程序主要包括以下部分：</br>
<ul>
	<li>预处理器指令</li>
	<li>函数</li>
...
<strong>标识符</strong></br>
C 标识符是用来标识变量、函数，或任何其他用户自定义项目的名称。一个标识符以字母 A-Z 或 a-z 或下划线 _ 开始，后跟零个或多个字母、下划线和数字（0-9）。
</br>
C 标识符内不允许出现标点字符，比如 @、$ 和 %。C 是区分大小写的编程语言。因此，在 C 中，Manpower 和 manpower 是两个不同的标识符。</br>
<strong>C 数据类型</strong></br>
<img src="images/c1.png"/></br>
<strong>定义常量</strong></br>
在 C 中，有两种简单的定义常量的方式：</br>
<ul>
	<li>使用 #define 预处理器。</li>
	<li>使用 const 关键字。</li>
</ul>
<strong>C 存储类</strong></br>
存储类定义 C 程序中变量/函数的范围（可见性）和生命周期。这些说明符放置在它们所修饰的类型之前。下面列出 C 程序中可用的存储类：</br>
<ul>
	<li>auto 存储类是所有局部变量默认的存储类。</li>
	<li>register 存储类用于定义存储在寄存器中而不是 RAM 中的局部变量</li>
	<li>static 存储类指示编译器在程序的生命周期内保持局部变量的存在，而不需要在每次它进入和离开作用域时进行创建和销毁。因此，使用 static 修饰局部变量可以在函数调用之间保持局部变量的值。</li>
	<li>extern 存储类用于提供一个全局变量的引用，它对所有的程序文件都是可见的。当您使用 'extern' 时，对于无法初始化的变量，会把变量名指向一个之前定义过的存储位置。</li>
</ul>
<strong>C 运算符</strong>
运算符是一种告诉编译器执行特定的数学或逻辑操作的符号。C 语言内置了丰富的运算符，并提供了以下类型的运算符：</br>
<ul>
<li>算术运算符</li>
<li>关系运算符</li>
<li>逻辑运算符</li>
...
<strong>初始化局部变量和全局变量:当局部变量被定义时，系统不会对其初始化，您必须自行对其初始化。定义全局变量时，系统会自动对其初始化</strong>
