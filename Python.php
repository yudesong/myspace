...
<div class="entry">
<p><strong>编译/执行 Python程序</strong></p></p>
<span>对于大多数编程语言，第一个入门实例便是 "Hello World!"，下面的实例使用 Python输出 "Hello World!"：</span>
<pre class="brush:javascript;gutter:true">
#!/usr/bin/ruby
print("Hello World!");
</pre>
<p><strong>Python 的特性</strong></p>
<span>
<ul>
	<li>易于学习：Python有相对较少的关键字，结构简单，和一个明确定义的语法，学习起来更加简单。</li>
	<li>易于阅读：Python代码定义的更清晰。</li>
	<li>易于维护：Python的成功在于它的源代码是相当容易维护的。</li>
	<li>广泛的标准库：Python的最大的优势之一是丰富的库，跨平台的，在UNIX，Windows和Macintosh兼容很好。</li>
	<li>互动模式：互动模式的支持，您可以从终端输入并获得结果的语言，互动的测试和调试代码片断。</li>
	<li>携式：Python可以运行在多种硬件平台和所有平台上都具有相同的接口。</li>
	<li>可扩展：可以添加低层次的模块到Python解释器。这些模块使程序员可以添加或定制自己的工具，更有效。</li>
	<li>数据库：Python提供所有主要的商业数据库的接口。</li>
	<li>GUI编程：Python支持GUI可以创建和移植到许多系统调用。</li>
	<li>可扩展性：相比 shell 脚本，Python 提供了一个更好的结构，且支持大型程序。</li>
</ul>
</span>
<p><strong>交互式 Python</strong></p>
<span>只需要在命令提示符中键入 python，一个交互式 Python将会开始，如下所示：</span>
<pre class="brush:javascript;gutter:true">
printf("hello world")
</pre>
<span>编译python文件：Python test.py</span></br>
<span>解决中文问题</span></br>
<pre class="brush:javascript;gutter:true">
#coding=utf-8
#!/usr/bin/python
...
<strong>Python标识符</strong></br>
<span>
在python里，标识符有字母、数字、下划线组成。</br>
在python中，所有标识符可以包括英文、数字以及下划线（_），但不能以数字开头。</br>
python中的标识符是区分大小写的。</br>
以下划线开头的标识符是有特殊意义的。以单下划线开头（_foo）的代表不能直接访问的类属性，需通过类提供的接口进行访问，不能用"from xxx import *"而导入；</br>
以双下划线开头的（__foo）代表类的私有成员；以双下划线开头和结尾的（__foo__）代表python里特殊方法专用的标识，如__init__（）代表类的构造函数。</br>
</span></br>
<strong>标准数据类型</strong></br>
<ul>
	<li>Numbers（数字）</li>
	<li>String（字符串）</li>
	<li>List（列表）</li>
