...
<span>显示一个模式对话框</span></br>
<pre class="brush:javascript;gutter:true">
CShowDlg dlg;
dlg.DoModal();	//显示模式对话框
CDialog::OnCancel();	//关闭模式对话框
</pre>
<span>显示一个非模式对话框</span></br>
<pre class="brush:javascript;gutter:true">
CShowDlg *dlg=new CShowDlg;
dlg->Create(IDD_SHOW_DIALOG,this);	//创建非模式对话框
dlg->ShowDialog(SW_SHOW);	//显示非模式对话框

dlg->DestoryWindow();	//销毁非模式对话框
delete dlg;
</pre>
<span>消息对话框</span></br>
<pre class="brush:javascript;gutter:true">
if(MessageBox("确定要退出应用程序？","系统提示",MB_OKCANCEL|MB_ICONQUESTION)!=IDOK)
{
	return ;
	CDialog::OnCancel();
}
</pre>
<span>Windows 通用对话框</span></br>
<span>文件对话框</span></br>
<pre class="brush:javascript;gutter:true">
...
<span>设置文本单击事件</span></br>
选择Notify属性在BN_CLICKED消息的处理函数
...
<span>使用按钮显示图标</span>
<pre class="brush:javascript;gutter:true">
m_ok.SetIcon(AfxGetApp()->LoadIcon(IDR_MAINFRAME));
</pre>
<span>复选框控件</span></br>
<span>设置复选框控件的选中状态</span>
<pre class="brush:javascript;gutter:true">
m_chinese.EnableWindow(FALSE);
m_math.SetCheck(1);
</pre>
<span>使用复选框控件统计信息</span>
<pre class="brush:javascript;gutter:true">
if(m_chinese.GetCheck()==1)
{
	MessageBox("已选中语文");
}
</pre>
...
<span>列表框控件</span></br>
<pre class="brush:javascript;gutter:true">
m_list.InsertString(int pos,CString str);
m_list.AddString("xxxx");
m_list.GetText(int pos,CString str);
